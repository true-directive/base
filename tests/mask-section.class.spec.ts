...
import { MaskSection, MaskSectionAction } from "../src/mask/mask-section.class";
...
describe(`MaskValue [dd.mm.yyyy] - split second section of "13.12.2018": `, () => {
  ...
  beforeEach(() => {
    ...
    const m: Mask = new Mask();
    m.setLocale(intl.locale);
    const sType = m.selectSectionType("mm");

    const section = new MaskSection(settings, "mm", ".", sType);
...
describe(`Section features`, () => {
  ...
  it(`Right arrow key at the end of the section`, () => {
    ...
    expect(res3.action).toBe(MaskSectionAction.GO_FWD);
  });
  ...
  it(`Backspace key at the start of the month`, () => {
    ...
    expect(res3.action).toBe(MaskSectionAction.GO_BACK_AND_DELETE);
  });

  it(`Backspace key at the start of text`, () => {
    let res3 = section.applyKey('31 jan 2019', Keys.BACKSPACE, '', 0, 0, 1);
    expect(res3.action).toBe(MaskSectionAction.NONE);
  });
});
